# desafio-mercearia-seu-ze

João Vitor Stainzack Figueiredo

Objetivo: Estágio na área de Tecnologia da Informação.

Formação: Sistemas de Informação — Universidade Positivo (UP), 1º período.

Conhecimentos: C, HTML, CSS, JavaScript, lógica de programação, algoritmos e UML.

Experiência: Sem experiência profissional prévia.

Perfil: Estudante interessado em tecnologia, programação e desenvolvimento profissional, buscando a primeira 

oportunidade na área.

RAFAEL MACIEL

19 anos
Curitiba – PR
Telefone: 41988956648
E-mail: rafaelmaciel896@gmail.com

OBJETIVO

Busco minha primeira oportunidade na área de Tecnologia da Informação, especialmente em desenvolvimento, suporte ou áreas relacionadas. Atualmente curso Análise e Desenvolvimento de Sistemas e procuro uma oportunidade para adquirir experiência prática, desenvolver meus conhecimentos e crescer profissionalmente na área de tecnologia.

FORMAÇÃO ACADÊMICA

Análise e Desenvolvimento de Sistemas (ADS)

CONHECIMENTOS

- Lógica de programação — básico
- Linguagem C — básico
- HTML e CSS — básico
- Banco de dados — noções básicas
- Microsoft Word e Excel — básico

Atualmente em processo de aprendizado e desenvolvimento dos conhecimentos técnicos.

EXPERIÊNCIA PROFISSIONAL

Atendimento / Telemarketing

- Atendimento e comunicação com clientes;
- Trabalho em equipe;
- Cumprimento de metas e procedimentos;
- Organização e responsabilidade com as atividades;
- Adaptação a diferentes demandas e situações.

HABILIDADES

- Facilidade para aprender
- Interesse por tecnologia
- Trabalho em equipe
- Boa comunicação
- Organização
- Proatividade
- Facilidade de adaptação
- Vontade de aprender e desenvolver novas habilidades

PERFIL PROFISSIONAL

Estudante de Análise e Desenvolvimento de Sistemas, atualmente desenvolvendo meus primeiros conhecimentos na área de tecnologia. Tenho facilidade para aprender, interesse em programação e vontade de adquirir experiência prática. Busco uma oportunidade que me permita aprender com profissionais da área, contribuir com a equipe e evoluir profissionalmente.
